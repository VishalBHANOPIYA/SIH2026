...
import type { CaseClassification, CaseStatus, User } from '../types';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, var(--color-surface-1) 0%, rgba(26,26,31,0.8) 100%)',
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-6)', fontSize: 'var(--text-sm)' }}>
            ...
            <div>
              ...
              <span style={{ color: 'var(--color-ink-muted)', fontWeight: 'var(--font-weight-medium)' }}>
                📁 {c.document_count} Attached Files
              </span>
            </div>
          </div>
      ...
      <div>
        {activeTab === 'documents' && (
          <div className="feature-card" style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <h3 className="card-title">Case Documents Vault</h3>
                <p className="text-muted" style={{ fontSize: 'var(--text-xs)', marginTop: 'var(--space-1)' }}>
                  Tamper-evident evidence media, FIR filings & forensic reports
                </p>
              </div>
              <button className="btn-primary" onClick={() => addToast('info', 'Document upload module will be wired up in the next module.')}>
                ＋ Upload Document
              </button>
            </div>
            ...
            {c.document_count === 0 ? (
              ...
              >
                ...
                <h4 style={{ fontSize: 'var(--text-base)', color: 'var(--color-ink)' }}>No documents attached yet</h4>
                <p className="text-muted" style={{ fontSize: 'var(--text-xs)', maxWidth: '400px' }}>
                  Upload FIR statements, witness testimonies, forensic reports, or court filings to generate cryptographic ledger proof.
                </p>
              </div>
            ) : (
              <div style={{ color: 'var(--color-ink-muted)', fontSize: 'var(--text-sm)' }}>
                {c.document_count} documents available.
              </div>
            )}
          </div>
        )}
